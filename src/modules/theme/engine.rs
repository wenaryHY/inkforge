...
pub async fn build_template_engine(state: Arc<AppState>) -> AppResult<Environment<'static>> {
    ...
    env.add_function("theme_assets_url", move |path: String| -> Result<Value, minijinja::Error> {
    ...
    env.add_function("get_recent_posts", move |limit: Option<i64>| -> Result<Value, minijinja::Error> {
    ...
    env.add_function("get_tags", move || -> Result<Value, minijinja::Error> {
        ...
        let result = tokio::task::block_in_place(move || {
            tokio::runtime::Handle::current().block_on(async move {
                crate::modules::tag::repository::list_tags(&pool).await
            })
        });
        match result {
    ...
    env.add_function("get_categories", move || -> Result<Value, minijinja::Error> {
